...
export default function CursoNovo() {
  ...
  const [dragAula, setDragAula] = useState(null);   // { modIdx, aulaIdx }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const slug = getSlug(form.titulo);
      await setDoc(doc(db, 'cursos', slug), {
        ...form, id: slug,
        modulos: modulos.map(({ open, ...m }) => ({
          ...m,
          aulas: m.aulas.map(a => ({
            ...a,
            slug: getSlug(a.titulo),
            tipo: a.tipo || 'rise360',
          })),
        })),
  ...
  const cursoSlug = getSlug(form.titulo);
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div className="form-group" style={{ gridColumn: '1/-1' }}>
              ...
              <input className="input" value={form.titulo} onChange={e => set('titulo', e.target.value)}
                placeholder="Ex: Pensamento Computacional" required />
              {form.titulo && <span style={{ fontSize: '0.75rem', color: 'var(--gray-500)', marginTop: '0.2rem' }}>ID: {cursoSlug}</span>}
            </div>
            <div className="form-group" style={{ gridColumn: '1/-1' }}>
              <label>Subtítulo</label>
              <input className="input" value={form.subtitulo} onChange={e => set('subtitulo', e.target.value)}
                placeholder="Ex: Lógica e Resolução de Problemas" />
            </div>
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <input className="input" value={form.cor} onChange={e => set('cor', e.target.value)} placeholder="#0088ff" />
              </div>
            </div>
            <div className="form-group">
              <label>Ícone</label>
                  ...
                  {m.open && (
                    <div className="aulas-list">
                      {m.aulas.map((a, ai) => {
                        ...
                        const aulaSlugVal = getSlug(a.titulo);
                        return (
                          <div
                            key={a.id}
                            draggable
                          ...
                          >
                            <span className="grip-handle" style={{ cursor: 'grab' }}><GripVertical size={14} /></span>
                            ...
                            <Rise360Upload
                              ...
                              aulaSlug={aulaSlugVal || null}
                              aulaUrl={a.arquivoUrl}
